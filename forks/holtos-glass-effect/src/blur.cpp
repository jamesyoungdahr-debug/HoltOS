...
#include "blur.h"
// In-tree this comes in through KWin's build; out of tree it is the installed
// header, and KWIN_BUILD_X11 below must see it.
...
Q_LOGGING_CATEGORY(KWIN_BLUR, "kwin_effect_holtosglass", QtWarningMsg)

static void ensureResources()
{
    // Must initialize resources manually because the effect is a static lib.
    Q_INIT_RESOURCE(holtosglass);
}
...
BlurEffect::BlurEffect()
{
    ...
    m_onscreenPass.shader = ShaderManager::instance()->generateShaderFromFile(ShaderTrait::MapTexture,
                                                                              QStringLiteral(":/effects/holtosglass/shaders/vertex.vert"),
                                                                              QStringLiteral(":/effects/holtosglass/shaders/onscreen.frag"));
    ...
    m_roundedOnscreenPass.shader = ShaderManager::instance()->generateShaderFromFile(ShaderTrait::MapTexture,
                                                                                     QStringLiteral(":/effects/holtosglass/shaders/onscreen_rounded.vert"),
                                                                                     QStringLiteral(":/effects/holtosglass/shaders/onscreen_rounded.frag"));
    ...
    m_downsamplePass.shader = ShaderManager::instance()->generateShaderFromFile(ShaderTrait::MapTexture,
                                                                                QStringLiteral(":/effects/holtosglass/shaders/vertex.vert"),
                                                                                QStringLiteral(":/effects/holtosglass/shaders/downsample.frag"));
    ...
    m_upsamplePass.shader = ShaderManager::instance()->generateShaderFromFile(ShaderTrait::MapTexture,
                                                                              QStringLiteral(":/effects/holtosglass/shaders/vertex.vert"),
                                                                              QStringLiteral(":/effects/holtosglass/shaders/upsample.frag"));
    ...
    m_noisePass.shader = ShaderManager::instance()->generateShaderFromFile(ShaderTrait::MapTexture,
                                                                           QStringLiteral(":/effects/holtosglass/shaders/vertex.vert"),
                                                                           QStringLiteral(":/effects/holtosglass/shaders/noise.frag"));
...
void BlurEffect::updateBlurRegion(EffectWindow *w)
{
    ...
    if (m_forceBlur && shouldForceBlur(w)) {
        ...
        // Deliberately not gated on decorationHasAlpha(): KWin 6.7 reports
        // the HoltOS (Klassy-based) decoration as opaque even when its
        // title bar is painted at 70 %, so the stock path never blurred
        // behind it (confirmed on screen 2026-09-12: a 30 % title bar
        // showed the raw background, untinted). Blurring under a truly
        // opaque frame costs a little and shows nothing; not blurring
        // under a translucent one is the bug.
