...
#include <linux/dmi.h>
/* HoltOS: built out of tree, so mach-config.h sits next to this file. */
...
/*
 * HoltOS: ASUS ROG Flow Z13 (GZ302). Its firmware sets acp-audio-config-flag
 * to FLAG_AMD_LEGACY_ONLY_DMIC, which would hand the chip to snd_acp_pci, but
 * that driver's microphone path records only garbage on this ACP 7.0 board.
 * Its _WOV (wake on voice) returns 0 and the PDM device (_ADR 0x02) has no
 * acp-audio-device-type property. On matching machines this driver takes the
 * chip despite the flag, ignores _WOV and accepts the PDM device, like the
 * upstream ASUS ACP 7.0 overrides (acp70_acpi_flag_override_table) and the
 * ThinkPad P14s Gen 6 AMD _WOV quirk. DMI_MATCH is a substring match: "ASUS"
 * covers "ASUSTeK COMPUTER INC." and the Z13-KJP's "ASUS", "GZ302EA" covers
 * GZ302EAC.
 */
static const struct dmi_system_id holtos_ps_force_dmic_table[] = {
	{
		.matches = {
...
{
	bool holtos_force_dmic = dmi_check_system(holtos_ps_force_dmic_table);
	...
	if (acp_data->is_pdm_config) {
		...
		if (pdm_dev) {
			...
			if (!acpi_dev_get_property(pdm_dev, "acp-audio-device-type",
				...
				dmic_en = true;
			else if (holtos_force_dmic)
				dmic_en = true;
		}

		handle = ACPI_HANDLE(&pci->dev);
		ret = acpi_evaluate_integer(handle, "_WOV", NULL, &dmic_status);
		if (!ACPI_FAILURE(ret) && !holtos_force_dmic)
			wov_en = dmic_status;
	}
	...
	if (holtos_force_dmic)
		dev_info(&pci->dev,
			 "HoltOS GZ302 microphone quirk (snd_pci_ps): pdm config %d, pdm device %d\n",
			 acp_data->is_pdm_config, is_dmic_dev);
			   ...
			   const struct pci_device_id *pci_id)
{
	...
	/* HoltOS: the GZ302 is driven here despite its legacy DMIC flag (see above). */
	if (flag && !dmi_check_system(holtos_ps_force_dmic_table))
		return -ENODEV;
