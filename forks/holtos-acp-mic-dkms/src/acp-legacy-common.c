...
/*
 * HoltOS: ASUS ROG Flow Z13 (GZ302). The firmware sets acp-audio-config-flag
 * to FLAG_AMD_LEGACY_ONLY_DMIC, but its _WOV method always returns 0 and the
 * PDM device (_ADR 0x02) has no acp-audio-device-type property, so the
 * built-in digital microphones were never registered. The same idea as the
 * upstream ThinkPad P14s Gen 6 AMD quirk (acp_ignore_wov_table in
 * ps/pci-ps.c). DMI_MATCH is a substring match: "ASUS" covers both
 * "ASUSTeK COMPUTER INC." and the Z13-KJP's "ASUS", and "GZ302EA" covers
 * GZ302EAC.
 */
static const struct dmi_system_id holtos_acp_force_dmic_table[] = {
	{
		.matches = {
...
{
	bool holtos_force_dmic = dmi_check_system(holtos_acp_force_dmic_table);
	...
	if (holtos_force_dmic)
		dev_info(&pci->dev,
			 "HoltOS GZ302 microphone quirk: pin config 0x%x, pdm config %d, pdm device %d\n",
			 readl(chip->base + ACP_PIN_CONFIG), chip->is_pdm_config, chip->is_pdm_dev);
}
