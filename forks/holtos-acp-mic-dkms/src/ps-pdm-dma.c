...
/* HoltOS test settings for the ROG Flow Z13 microphones (holtos-mic-test). */
static int holtos_clk_ctrl = -1;
module_param(holtos_clk_ctrl, int, 0644);
MODULE_PARM_DESC(holtos_clk_ctrl, "HoltOS test: value for ACP_WOV_CLK_CTRL at capture start (-1 = driver default)");
static int holtos_clkdiv = -1;
module_param(holtos_clkdiv, int, 0644);
MODULE_PARM_DESC(holtos_clkdiv, "HoltOS test: value for ACP_PDM_CLKDIV at capture start (-1 = not written)");
...
static void acp63_enable_pdm_clock(void __iomem *acp_base)
{
	...
	/* HoltOS test: the clock register values before and after, and the
	 * holtos_clk_ctrl / holtos_clkdiv test settings (-1 = stock behaviour). */
	pr_info("snd_ps_pdm_dma: HoltOS test: before: CLK_CTRL 0x%x CLKDIV 0x%x MISC_CTRL 0x%x\n",
		readl(acp_base + ACP_WOV_CLK_CTRL), readl(acp_base + ACP_PDM_CLKDIV),
		readl(acp_base + ACP_WOV_MISC_CTRL));

	pdm_clk_enable = holtos_clk_ctrl >= 0 ? holtos_clk_ctrl : ACP_PDM_CLK_FREQ_MASK;
	pdm_ctrl = 0x00;

	if (holtos_clkdiv >= 0)
		writel(holtos_clkdiv, acp_base + ACP_PDM_CLKDIV);
	...
	pr_info("snd_ps_pdm_dma: HoltOS test: after (clk_ctrl %d, clkdiv %d): CLK_CTRL 0x%x CLKDIV 0x%x MISC_CTRL 0x%x\n",
		holtos_clk_ctrl, holtos_clkdiv,
		readl(acp_base + ACP_WOV_CLK_CTRL), readl(acp_base + ACP_PDM_CLKDIV),
		readl(acp_base + ACP_WOV_MISC_CTRL));
}
