...
// HoltOS glass (G7): Qt Quick windows get an alpha buffer so Kirigami apps can
// paint translucent backgrounds, with KWin's blur requested behind them in
// KWaylandIntegration. It must happen here, inside the QGuiApplication
// constructor, before any QQuickWindow exists. Plasma's own shell, KWin,
// the splash, lock screen, login greeter and portals manage their own
// surfaces and are left alone; HOLTOS_GLASS=0 switches it off for any app.
static bool holtosGlassWanted()
{
    if (qEnvironmentVariable("HOLTOS_GLASS") == QLatin1String("0")) {
        return false;
    }
    if (QGuiApplication::platformName() != QLatin1String("wayland")) {
        return false;
    }
    const QString exe = QFileInfo(QStringLiteral("/proc/self/exe")).symLinkTarget().section(QLatin1Char('/'), -1);
    static const QStringList excluded = {
        ...
        QStringLiteral("ksmserver-logout-greeter"),
    ...
    return !exe.isEmpty() && !excluded.contains(exe);
}

KdePlatformTheme::KdePlatformTheme()
{
    if (holtosGlassWanted()) {
        QQuickWindow::setDefaultAlphaBuffer(true);
        qApp->setProperty("holtos.glass", true);
    }
