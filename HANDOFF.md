...
update; the glass installer (`pkexec calamares -style kvantum`, a system-wide
Kvantum config, RGBA stylesheet; the QML bars stay opaque because
QQuickWidget cannot be translucent); driver detection for the installer and
...
Open: rename "homelab" to HoltOS (keep the `[homelab]` package repo reachable
during a migration), install alongside Windows (small ESP), rebuild and
VM-install the ISO (`holtos-rebrand` VM), hardware checks from LIVETEST.md's
rebrand section, and Liam's pick of the version number.
