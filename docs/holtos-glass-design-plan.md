...
| G6 | GTK3 rgba css. **Done 2026-09-15 on `neon-rebrand`: translucent HoltOS Glass gtk.css for GTK 3 and GTK 4/libadwaita in /etc/skel (window 72 %, views 55 %, menus and popovers 88 %); holtos-glass-user-update step 9 copies them to accounts with no gtk.css or an unchanged HoltOS one. Relies on KWin ForceBlur for the blur. Not seen on a desktop yet** | Liamtab |
...
| G8 | Adaptive contrast, step 1: bind the smallest blur level, debug view of L. **Built 2026-09-15 together with G9 on holtos-kwin branch `adaptive-contrast` (commit 6482460, pushed, not merged into `holtos`): both on-screen shaders sample `framebuffers[m_iterationCount]` on texture unit 1. No debug view. Compiled as holtos-kwin 6.7.5.r0 in the build container, kept out of every package repo (copy in `out/adaptive-contrast/`); DeepSeek Pro review found no defects** | compile, then Liamtab |
| G9 | Adaptive contrast, step 2: clamp and kcfg keys, blur_config.ui. **Built 2026-09-15 (see G8): smoothstep-eased clamp in approximately linear light; kcfg `AdaptiveContrast` (default false) and `AdaptiveMaxLuminance` (percent, default 30). No blur_config.ui controls yet. To test on Liamtab: install the package, set `[Effect-blur] AdaptiveContrast=true` in kwinrc, `qdbus6 org.kde.KWin /KWin reconfigure`** | Liamtab, window over a bright wallpaper, check 4.5:1 under text |
